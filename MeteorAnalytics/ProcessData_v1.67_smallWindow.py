...
def plotDist(aMax, aSum, tDur, tDist):
    ...
    plt.grid(b=True, which="major", axis="x", linestyle="dotted")
    ...
    if len(tDist) < 2:
        ...
        binNr = 4 * int(sc.log10(len(tDist)) / sc.log10(2))
    # Histogramm zeichnen
...
def compileDist():
    ...
    aMax = []
    for line in ShortList:
        aMax.append(sc.log10(sc.sqrt(line[0][4])))
    # Extrahiere Powersummen-Spalte und wandle in Power in Amplitude um
    aSum = []
    for line in ShortList:
        aSum.append(sc.log10(sc.sqrt(line[1])))
...
def writeDist(n1, m1, n2, m2):
    ...
    for i in range(len(n1)):
        ...
        d.write(str(classM) + "," + str(sc.log10(n1[i])) + "\r")
...
dataTable = tkinter.scrolledtext.ScrolledText(main, width=75, height=14)
